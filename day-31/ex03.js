...
function displayTarotReading(card) {
  const tarotReadingDiv = document.getElementById("tarotReading");
  tarotReadingDiv.innerHTML = "";
  const cardDiv = document.createElement("div");
  cardDiv.className = "tarot-card";
  cardDiv.innerHTML = `<h3>${card.name}</h3><p>${card.description}</p>`;
  tarotReadingDiv.appendChild(cardDiv);
}

document.getElementById("drawCard").addEventListener("click", () => {
  const drawnCard = drawRandomCard();
  displayTarotReading(drawnCard);

  document.getElementById("endDraw").classList.remove("hidden");
});

document.getElementById("endDraw").addEventListener("click", () => {
  alert("Bạn đã kết thúc bói bài");
  document.getElementById("tarotReading").innerHTML = "";
  document.getElementById("endDraw").classList.add("hidden");
});
