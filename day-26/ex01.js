function printHighlight(content, keyword) {
  if (typeof keyword !== "string" || content.length < keyword.length) {
    return "Dữ liệu không hợp lệ";
  }
  let lowerKeyword = keyword.toLowerCase();
  const highlightedKeyword = `<b>${lowerKeyword}</b>`;
  const highlightedContent = content.replaceAll(keyword, highlightedKeyword);
...
function fixContent(content) {
  ...
  // Thêm dấu cách sau các dấu câu
  content = content.replace(/([.,;!?])/g, "$1 ");

  // Thêm dấu cách trước các ký tự mở và sau các ký tự đóng đặc biệt
  content = content.replace(/([({<])/g, " $1").replace(/([)}>])/g, "$1 ");

  // Loại bỏ các dấu cách dư thừa (nếu có)
